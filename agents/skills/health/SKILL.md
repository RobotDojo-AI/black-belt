---
name: health
description: Deprecated. Use `/topic health`. This stub remains as a redirect so live `/health` invocations are not silently broken.
canonical_reads:
...
`/health` is deprecated. Use `/topic health` instead.

## Contract

This skill is a redirect. It loads no substrate, writes no artifact, and stops with a one-line message pointing the caller at `/topic health`.

The health corpus, analytical framing (see `framings/medical-engineer.md`), current medications, and recent markers live under `user/contexts/topics/personal/health/` (canonical topic context substrate, with `framings/medical-engineer.md` as the load-bearing analytical framing). `/topic health` resolves the topic, loads `context.md` + framings + current med/marker state + the latest synthesis, and runs the same OPEN/CLOSE ritual the bespoke `/health` used to embody.

## Redirect message

```text
/health is deprecated. Use:

  /topic health

The health corpus + analytical framing (see `framings/medical-engineer.md`) are loaded automatically by `/topic`.
```
